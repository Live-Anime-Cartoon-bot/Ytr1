...
async def cancelme_cmd(client: Client, message: Message):
    user_id = message.from_user.id
    raw_args = (message.text or "").split()
    if len(raw_args) == 2 and raw_args[1].isdigit():
        _scheduler_init()
        jid = int(raw_args[1])
        row = next((r for r in _schedule_rows(user_id) if r["id"] == jid), None)
        if row:
            _schedule_update(jid, "cancelled")
            t = _SCHEDULER_TASKS.pop(jid, None)
            if t: t.cancel()
            return await message.reply_text(f"✅ Schedule `{jid}` cancelled.")
    recs    = active_recs.get(user_id, {})
...
# ---------------------------------------------------------------------------
# DIRECT RECORDING — isolated task manager + safe FFmpeg argument arrays
# ---------------------------------------------------------------------------

_DIRECT_TASKS: dict[str, dict] = {}
_DIRECT_UPDATE_INTERVAL = 5.0
_DIRECT_DEFAULT_FONT = _environ.get("WATERMARK_FONT", "/system/fonts/Roboto-Regular.ttf")
_DIRECT_WM_IMAGE_URL = "https://iili.io/CbTi4Nn.png"
_DIRECT_ADVANCE_INTERVALS = [(10, 50), (1200, 1260), (1800, 1860)]


def _safe_filename(raw: str, fallback: str = "recording.mp4") -> str:
    """Return a basename-only .mp4 filename; reject traversal/path tricks."""
...
def _authorized_url(url: str) -> bool:
    """Allow only exact catalog URLs or explicitly configured source hosts."""
    try:
        parsed = urllib.parse.urlparse(url.strip())
        if parsed.scheme not in {"http", "https"} or not parsed.netloc:
            return False
        exact = {u for u in get_public_channels().values()}
        if url.strip() in exact:
            return True
        configured = set()
        for item in (_environ.get("AUTHORIZED_SOURCE_HOSTS", "") or "").split(","):
            host = item.strip().lower()
            if host:
                configured.add(host)
        return parsed.hostname.lower() in configured if parsed.hostname else False
    except Exception:
        return False


def _safe_filter_text(text: str) -> str:
    # drawtext filter escaping: backslash, apostrophe, colon and percent.
...
def _text_watermark_filter(text: str, intervals=None) -> str:
    text = _safe_filter_text(text)
    enable = ""
    if intervals:
        expr = "+".join(f"between(t,{a},{b})" for a, b in intervals)
        enable = f":enable='{expr}'"
    return (
        f"{_fit_filter()},drawtext=fontfile='{_safe_filter_text(_DIRECT_DEFAULT_FONT)}':"
        f"text='{text}':fontsize=24:fontcolor=white:x=(w-tw)/2:y=h-th-140:"
        f"shadowcolor=black:shadowx=2:shadowy=2{enable}"
    )


def _direct_keyboard(task_id: str, started: bool = False) -> InlineKeyboardMarkup:
    if started:
        return InlineKeyboardMarkup([[InlineKeyboardButton("⚡ Progress", callback_data=f"progress:{task_id}"),
                                      InlineKeyboardButton("❌ Cancel", callback_data=f"cancel:{task_id}")]])
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🟢 Normal", callback_data=f"direct:{task_id}:normal"),
         InlineKeyboardButton("⚡ Advance", callback_data=f"direct:{task_id}:advance")],
        [InlineKeyboardButton("🚫 Watermark OFF", callback_data=f"direct:{task_id}:off"),
         InlineKeyboardButton("🖼️ Image Watermark", callback_data=f"direct:{task_id}:image")],
        [InlineKeyboardButton("❌ Cancel", callback_data=f"cancel:{task_id}")],
    ])


def _progress_bar(pct: float, n: int = 10) -> str:
    pct = max(0.0, min(100.0, pct))
    filled = int(pct / 100 * n)
    return "[" + "●" * filled + "○" * (n - filled) + "]"


def _direct_status_text(job: dict) -> str:
    pct = float(job.get("progress", 0.0))
    elapsed = max(0, int(time.time() - job["started_at"]))
    speed = job.get("speed") or "Calculating..."
    remain = "Calculating..."
    if pct > 0 and pct < 100 and elapsed:
        remain = f"{int(elapsed * (100 - pct) / pct)}s"
    return ("🎬 **Processing Video...**\n\n"
            f"📄 File:\n{job['filename']}\n\n"
            f"Progress:\n{_progress_bar(pct)} {pct:.2f}%\n\n"
            f"⚡ Speed:\n{speed}\n\n"
            f"Status:\n{job.get('status', 'Processing...')}\n\n"
            f"⏱ Elapsed: {elapsed}s\n⏳ Remaining: {remain}")


def _direct_popup(job: dict) -> str:
    u = job.get("user") or {}
    return (f"📄 File: {job['filename']}\n"
            f"Progress: {job.get('progress', 0.0):.2f}%\n"
            f"Status: {job.get('status', 'Processing...')}\n"
            f"Elapsed: {int(time.time()-job['started_at'])}s\n"
            f"Remaining: calculating\n"
            f"Speed: {job.get('speed') or 'Calculating...'}\n"
            f"Username: @{u.username}" if getattr(u, 'username', None) else
            f"📄 File: {job['filename']}\nProgress: {job.get('progress', 0.0):.2f}%\n"
            f"Status: {job.get('status', 'Processing...')}\nElapsed: {int(time.time()-job['started_at'])}s\n"
            f"Remaining: calculating\nSpeed: {job.get('speed') or 'Calculating...'}") + f"\nUser ID: {job['user_id']}"


async def _direct_updater(job: dict):
    try:
        while not job.get("done"):
            try:
                await job["status_msg"].edit_text(_direct_status_text(job), reply_markup=_direct_keyboard(job["task_id"], True))
            except Exception:
                pass
            await asyncio.sleep(_DIRECT_UPDATE_INTERVAL)
    except asyncio.CancelledError:
        return


async def _download_permitted_image(url: str, path: str) -> None:
    if url != _DIRECT_WM_IMAGE_URL and url not in {
        x.strip() for x in (_environ.get("AUTHORIZED_WATERMARK_IMAGES", "") or "").split(",") if x.strip()
    }:
        raise ValueError("Image URL is not an authorized/permitted watermark source.")
    await asyncio.to_thread(urllib.request.urlretrieve, url, path)


async def _run_direct_ffmpeg(job: dict, mode: str):
    out_path = job["output_path"]
    input_path = job["input_path"]
    duration = job["duration"]
    args = ["ffmpeg", "-hide_banner", "-y", "-progress", "pipe:1", "-nostats",
            "-i", input_path]
    filter_complex = None
    if mode == "image":
        img = join(job["save_dir"], "watermark.png")
        image_url = job.get("watermark_link") if (job.get("watermark_link") or "").lower().split("?", 1)[0].endswith((".png", ".jpg", ".jpeg", ".webp")) else _DIRECT_WM_IMAGE_URL
        await _download_permitted_image(image_url, img)
        filter_complex = f"[0:v]{_fit_filter()}[v0];[1:v]scale=260:-1[wm];[v0][wm]overlay=(W-w)/2:100[v]"
        args = ["ffmpeg", "-hide_banner", "-y", "-progress", "pipe:1", "-nostats",
                "-i", input_path, "-i", img, "-t", str(duration),
                "-filter_complex", filter_complex, "-map", "[v]", "-map", "0:a?",
                "-c:v", "libx264", "-preset", "ultrafast", "-crf", "23", "-threads", "2",
                "-c:a", "aac", "-movflags", "+faststart", out_path]
    else:
        text = job.get("watermark_text") or get_default_watermark()
        vf = _fit_filter() if mode == "off" else _text_watermark_filter(text, _DIRECT_ADVANCE_INTERVALS if mode == "advance" else None)
        args = ["ffmpeg", "-hide_banner", "-y", "-progress", "pipe:1", "-nostats",
                "-i", input_path, "-t", str(duration), "-map", "0:v:0", "-map", "0:a?",
                "-vf", vf, "-c:v", "libx264", "-preset", "ultrafast", "-threads", "2",
                "-c:a", "aac", "-movflags", "+faststart", out_path]
    ...
    try:
        while True:
            ...
            if "out_time_ms=" in raw:
                try:
                    sec = int(raw.split("=", 1)[1]) / 1_000_000
                    job["progress"] = min(100.0, sec * 100.0 / max(1, duration))
                except ValueError:
                    pass
            elif raw.startswith("speed="):
                job["speed"] = raw.split("=", 1)[1]
        rc = await proc.wait()
        err = (await stderr_task).decode(errors="ignore")
    finally:
    ...
    if rc != 0 or not os.path.exists(out_path) or os.path.getsize(out_path) < 1024:
        raise RuntimeError(err.strip()[-1800:] or f"FFmpeg exited with code {rc}")
    job["progress"] = 100.0


async def _start_direct_task(client: Client, task_id: str, mode: str):
    job = _DIRECT_TASKS.get(task_id)
    if not job or job.get("done"):
        return
    job["mode"] = mode
    job["status"] = "Processing..."
    job["updater"] = asyncio.create_task(_direct_updater(job))
    try:
        await _run_direct_ffmpeg(job, mode)
        job["status"] = "Completed"
        job["done"] = True
        try:
            await job["status_msg"].edit_text(f"✅ **Processing Completed**\n📄 File:\n{job['output_path']}\nUploading...", reply_markup=None)
        except Exception:
            pass
        caption = f"🎬 {job.get('watermark_text') or 'Video'}"
        await _run_upload_destination(client, job["user_id"], "tg", job["status_msg"], job["output_path"],
                                      caption, job["duration"], save_dir=job["save_dir"])
        ...
        try: await job["status_msg"].edit_text("❌ **Process Cancelled**\n⚠️ Partial Output Deleted", reply_markup=None)
        except Exception: pass
    except Exception as exc:
        job["status"] = "Failed"
        job["done"] = True
        ...
        LOG.exception("direct task %s failed: %s", task_id, exc)
        try: await job["status_msg"].edit_text(f"❌ **Processing Failed**\n⚠️ Partial Output Deleted\n📄 File:\n{job['filename']}", reply_markup=None)
        except Exception: pass
    finally:
        if job.get("updater"):
            job["updater"].cancel()
...
async def directrec_cmd(client: Client, message: Message):
    ...
    try:
        tokens = shlex.split(raw.split(None, 1)[1])
    except (IndexError, ValueError):
        tokens = []
    if len(tokens) < 2 or len(tokens) > 5:
        return await message.reply_text("Usage: `/drec <URL> <duration> <filename> [watermark_text] [watermark_link]`")
    url, duration_s = tokens[0], _parse_duration_seconds(tokens[1])
    if not _authorized_url(url):
        return await message.reply_text("❌ Source URL is not in the permitted source list.")
    if duration_s <= 0 or duration_s > 6 * 3600:
        return await message.reply_text("❌ Duration must be between 1 second and 6 hours.")
    try:
        filename = _safe_filename(tokens[2] if len(tokens) >= 3 else "recording.mp4")
    except ValueError as exc:
        return await message.reply_text(f"❌ {exc}")
    wm_text = tokens[3] if len(tokens) >= 4 else get_default_watermark()
    wm_link = tokens[4] if len(tokens) >= 5 else ""
    if wm_link and not re.fullmatch(r"https?://[^\s]+", wm_link):
        return await message.reply_text("❌ Invalid watermark link.")
    task_id = secrets_token = pysecrets.token_hex(8)
    save_dir = join(DOWNLOAD_DIRECTORY, f"direct_{task_id}")
    os.makedirs(save_dir, exist_ok=True)
    input_path = join(save_dir, "input.stream")
    output_path = join(save_dir, filename)
    job = {"task_id": task_id, "user_id": user_id, "user": message.from_user, "chat_id": message.chat.id,
           "status_msg": None, "input_path": input_path, "output_path": output_path, "filename": filename,
           "duration": duration_s, "watermark_text": wm_text, "watermark_link": wm_link, "save_dir": save_dir,
           "progress": 0.0, "speed": None, "status": "Waiting for mode...", "started_at": time.time(),
           "proc": None, "updater": None, "done": False, "cancelled": False}
    _DIRECT_TASKS[task_id] = job
    status = await message.reply_text(_direct_status_text(job), reply_markup=_direct_keyboard(task_id))
    job["status_msg"] = status
    # Store the remote URL separately; input is fetched by FFmpeg directly.
    job["input_path"] = url


@app.on_callback_query(filters.regex(r"^direct:([0-9a-f]+):(normal|advance|off|image)$"))
async def direct_mode_callback(client: Client, cq: CallbackQuery):
    task_id, mode = cq.matches[0].group(1), cq.matches[0].group(2)
    job = _DIRECT_TASKS.get(task_id)
    if not job or cq.from_user.id != job["user_id"]:
        return await cq.answer("Task not found or not yours.", show_alert=True, cache_time=0)
    if job.get("mode") or job.get("done"):
        return await cq.answer("Task already started.", show_alert=True, cache_time=0)
    ok, quota_msg = use_rec(job["user_id"])
    if not ok:
        return await cq.answer(quota_msg.replace("**", ""), show_alert=True, cache_time=0)
    await cq.answer("Starting...", show_alert=True, cache_time=0)
    job["mode"] = mode
    job["started_at"] = time.time()
    job["status"] = "Processing..."
    asyncio.create_task(_start_direct_task(client, task_id, mode))


@app.on_callback_query(filters.regex(r"^progress:([0-9a-f]+)$"))
async def direct_progress_callback(client: Client, cq: CallbackQuery):
    task_id = cq.matches[0].group(1)
    job = _DIRECT_TASKS.get(task_id)
    if not job or cq.from_user.id != job["user_id"]:
        return await cq.answer("Task not found or not yours.", show_alert=True, cache_time=0)
    await cq.answer(_direct_popup(job), show_alert=True, cache_time=0)


@app.on_callback_query(filters.regex(r"^cancel:([0-9a-f]+)$"))
async def direct_cancel_callback(client: Client, cq: CallbackQuery):
    task_id = cq.matches[0].group(1)
    job = _DIRECT_TASKS.get(task_id)
    if not job or cq.from_user.id != job["user_id"]:
        return await cq.answer("You do not have permission to cancel this task.", show_alert=True, cache_time=0)
    job["cancelled"] = True
    proc = job.get("proc")
    ...
    if job.get("updater"):
        job["updater"].cancel()
    await cq.answer("Cancellation requested.", show_alert=True, cache_time=0)
...
# ---------------------------------------------------------------------------
# /Watermark — reply-to-supported-media, per-task mode selection
# ---------------------------------------------------------------------------

_WM_TASKS: dict[str, dict] = {}


def _watermark_mode_keyboard(task_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("💧 Watermark 1", callback_data=f"wmstart:{task_id}:1")],
        [InlineKeyboardButton("🖼️ Watermark 2", callback_data=f"wmstart:{task_id}:2")],
        [InlineKeyboardButton("❌ Cancel", callback_data=f"wmcancel:{task_id}")],
    ])


def _watermark_source(message):
    src = message.reply_to_message
    if not src:
        return None
    if src.video:
        return src
    if src.document and (src.document.mime_type or "").lower().startswith("video/"):
        return src
    return None


async def _run_watermark_task(client: Client, task_id: str, mode: str):
    job = _WM_TASKS[task_id]
    status = job["status_msg"]
    save_dir = job["save_dir"]
    proc = None
    try:
        await status.edit_text("🎬 **Processing Video...**\n\n📄 File:\n" + job["filename"] + "\n\nProgress:\n[□□□□□□□□□□] 0.00%\n\n⚡ Speed:\nCalculating...\n\nStatus:\nDownloading...")
        src_path = await job["src_msg"].download(file_name=join(save_dir, "source.bin"))
        job["input_path"] = src_path
        info = await _ffprobe_video(src_path)
        duration = max(1, int(float(info.get("duration") or 0)))
        job["duration"] = duration
        out_path = join(save_dir, job["filename"])
        if mode == "1":
            vf = _text_watermark_filter("Join Our Telegram - AnimeCartoonPremium", _DIRECT_ADVANCE_INTERVALS)
            args = ["ffmpeg", "-hide_banner", "-y", "-progress", "pipe:1", "-nostats", "-i", src_path,
                    "-map", "0:v:0", "-map", "0:a?", "-vf", vf, "-c:v", "libx264", "-crf", "23",
                    "-preset", "ultrafast", "-threads", "2", "-c:a", "aac", "-movflags", "+faststart", out_path]
        else:
            img = join(save_dir, "watermark.png")
            await _download_permitted_image(_DIRECT_WM_IMAGE_URL, img)
            fc = f"[0:v]{_fit_filter()}[v0];[1:v]scale=260:-1[wm];[v0][wm]overlay=(W-w)/2:100[v]"
            args = ["ffmpeg", "-hide_banner", "-y", "-progress", "pipe:1", "-nostats", "-i", src_path,
                    "-i", img, "-map", "[v]", "-map", "0:a?", "-filter_complex", fc, "-c:v", "libx264",
                    "-crf", "23", "-preset", "ultrafast", "-threads", "2", "-c:a", "aac",
        ...
        while True:
            ...
            if text.startswith("out_time_ms="):
                try: job["progress"] = min(100.0, int(text.split("=",1)[1])/1_000_000*100/max(1,duration))
                except ValueError: pass
            elif text.startswith("speed="):
                job["speed"] = text.split("=",1)[1]
            if time.time()-last_edit >= 5:
                last_edit=time.time()
                try:
                    await status.edit_text(_direct_status_text(job), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⚡ Progress", callback_data=f"progress:{task_id}"), InlineKeyboardButton("❌ Cancel", callback_data=f"wmcancel:{task_id}")]]))
                except Exception: pass
        rc = await proc.wait()
        err = (await stderr_task).decode(errors="ignore")
        if job.get("cancelled"):
        ...
        job["progress"] = 100.0
        job["status"] = "Completed"
        await status.edit_text(f"✅ **Processing Completed**\n📄 File:\n{out_path}\nUploading...")
        await _run_upload_destination(client, job["user_id"], "tg", status, out_path,
                                      "🎬 Watermarked Video", duration, save_dir=save_dir)
        schedule_retention_cleanup(save_dir)
    except asyncio.CancelledError:
        if os.path.exists(job.get("output_path", "")):
            try: os.remove(job["output_path"])
            except OSError: pass
        try: await status.edit_text("❌ **Process Cancelled**\n⚠️ Partial Output Deleted", reply_markup=None)
        except Exception: pass
    except Exception as exc:
        LOG.exception("watermark task %s failed: %s", task_id, exc)
        if os.path.exists(job.get("output_path", "")):
            try: os.remove(job["output_path"])
            except OSError: pass
        try: await status.edit_text(f"❌ **Processing Failed**\n⚠️ Partial Output Deleted\n📄 File:\n{job['filename']}", reply_markup=None)
        except Exception: pass
    finally:
        job["done"] = True
        job["proc"] = None
...
async def watermark_cmd(client: Client, message: Message):
    ...
    src = _watermark_source(message)
    if not src:
        return await message.reply_text("❌ `/Watermark` must be used as a reply to a supported Telegram video/document.")
    filename_arg = ""
    parts = (message.text or "").split(None, 1)
    if len(parts) == 2:
        filename_arg = parts[1].strip()
    try:
        original = src.video.file_name if src.video else src.document.file_name
        filename = _safe_filename(filename_arg or original or "watermarked.mp4", "watermarked.mp4")
    except ValueError as exc:
        return await message.reply_text(f"❌ {exc}")
    task_id = pysecrets.token_hex(8)
    save_dir = join(DOWNLOAD_DIRECTORY, f"wm_{task_id}")
    os.makedirs(save_dir, exist_ok=True)
    status = await message.reply_text(
        f"🎬 **Processing Video...**\n\n📄 File:\n{filename}\n\nSelect watermark:",
        reply_markup=_watermark_mode_keyboard(task_id))
    _WM_TASKS[task_id] = {"task_id": task_id, "user_id": user_id, "src_msg": src, "filename": filename,
                           "save_dir": save_dir, "status_msg": status, "started_at": time.time(),
                           "progress": 0.0, "speed": None, "status": "Waiting for watermark...", "proc": None,
                           "done": False, "cancelled": False}


@app.on_callback_query(filters.regex(r"^wmstart:([0-9a-f]+):(1|2)$"))
async def watermark_start_callback(client: Client, cq: CallbackQuery):
    task_id, mode = cq.matches[0].group(1), cq.matches[0].group(2)
    job = _WM_TASKS.get(task_id)
    if not job or cq.from_user.id != job["user_id"]:
        return await cq.answer("Task not found or not yours.", show_alert=True, cache_time=0)
    if job.get("started"):
        return await cq.answer("Task already started.", show_alert=True, cache_time=0)
    ...
    job["started_at"] = time.time()
    await cq.answer("Starting...", show_alert=True, cache_time=0)
    asyncio.create_task(_run_watermark_task(client, task_id, mode))


@app.on_callback_query(filters.regex(r"^wmcancel:([0-9a-f]+)$"))
async def watermark_cancel_callback(client: Client, cq: CallbackQuery):
    task_id = cq.matches[0].group(1)
    job = _WM_TASKS.get(task_id)
    if not job or cq.from_user.id != job["user_id"]:
        return await cq.answer("You do not have permission to cancel this task.", show_alert=True, cache_time=0)
...
# ---------------------------------------------------------------------------
# Persistent SQLite scheduler (Asia/Kolkata)
# ---------------------------------------------------------------------------

SCHEDULER_DB = join(DATA_DIRECTORY, "scheduler.sqlite3")
_SCHEDULER_TASKS: dict[int, asyncio.Task] = {}


def _scheduler_init():
    con = sqlite3.connect(SCHEDULER_DB)
...
def _schedule_add(user_id, channel, platform, date, start_time, end_time, video, audio):
    con=sqlite3.connect(SCHEDULER_DB)
    cur=con.execute("INSERT INTO schedules(user_id,channel,platform,date,start_time,end_time,video,audio,status,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)",
                    (user_id,channel,platform,date,start_time,end_time,video,audio,"pending",datetime.now(tz).isoformat()))
    con.commit(); job_id=cur.lastrowid; con.close(); return job_id


def _schedule_rows(user_id=None):
    con=sqlite3.connect(SCHEDULER_DB); con.row_factory=sqlite3.Row
    if user_id is None: rows=con.execute("SELECT * FROM schedules ORDER BY id DESC").fetchall()
    else: rows=con.execute("SELECT * FROM schedules WHERE user_id=? ORDER BY id DESC",(user_id,)).fetchall()
    con.close(); return [dict(r) for r in rows]


def _schedule_update(job_id, status):
    con=sqlite3.connect(SCHEDULER_DB); con.execute("UPDATE schedules SET status=? WHERE id=?",(status,job_id)); con.commit(); con.close()


@app.on_message(filters.command(["Myschedule", "myschedule"]) & AUTH)
async def myschedule_cmd(client: Client, message: Message):
    _scheduler_init()
    uid=message.from_user.id
    args=(message.text or "").split()
    if len(args)==1:
        rows=_schedule_rows(uid)
        if not rows: return await message.reply_text("📅 No schedules found.")
        lines=["📅 **My Schedule**"]
        for r in rows[:30]: lines.append(f"#{r['id']} | {r['channel']} | {r['date']} {r['start_time']}-{r['end_time']} | {r['status']}")
        return await message.reply_text("\n".join(lines))
    if len(args)<6:
        return await message.reply_text("Usage: `/Myschedule <channel> <DD-MM-YYYY> <start> <end> <filename>`\nExample: `/Myschedule POGO 28-08-2026 10:00 11:00 Pogo.mp4`")
    channel=args[1]; date_s=args[2]; start_s=args[3]; end_s=args[4]; filename=args[5]
    if not get_channel_url(channel): return await message.reply_text("❌ Channel is not in the permitted catalog.")
    try:
        datetime.strptime(date_s,"%d-%m-%Y"); datetime.strptime(start_s,"%H:%M"); datetime.strptime(end_s,"%H:%M")
        filename=_safe_filename(filename)
    except Exception: return await message.reply_text("❌ Invalid date/time/filename.")
    ok, quota_msg = use_rec(uid)
    if not ok:
        return await message.reply_text(quota_msg)
    jid=_schedule_add(uid,channel,"public",date_s,start_s,end_s,filename,"default")
    await message.reply_text(f"✅ Schedule created. Job ID: `{jid}`")
...
async def cancel_schedule_cmd(client: Client, message: Message):
    ...
    if len(args)!=2 or not args[1].isdigit():
        return await message.reply_text("Usage: `/cancel <job_id>`")
    jid=int(args[1]); rows=_schedule_rows(message.from_user.id); row=next((r for r in rows if r["id"]==jid),None)
    if not row: return await message.reply_text("❌ Schedule not found.")
    _schedule_update(jid,"cancelled")
    t=_SCHEDULER_TASKS.pop(jid,None)
    if t: t.cancel()
    await message.reply_text(f"✅ Schedule `{jid}` cancelled.")


async def _execute_schedule(client, row):
    try:
        date_s=row["date"]; start_s=row["start_time"]
        target=tz.localize(datetime.strptime(f"{date_s} {start_s}","%d-%m-%Y %H:%M"))
        delay=(target-datetime.now(tz)).total_seconds()
        if delay>0: await asyncio.sleep(delay)
        if row["status"]!="pending": return
        url=get_channel_url(row["channel"])
        if not url: raise RuntimeError("Permitted channel URL no longer available")
        duration=max(1,int((datetime.strptime(row["end_time"],"%H:%M")-datetime.strptime(start_s,"%H:%M")).total_seconds()))
        # Midnight-crossing schedule.
        if duration<=0: duration=24*3600+duration
        task_id=pysecrets.token_hex(8); save_dir=join(DOWNLOAD_DIRECTORY,f"sched_{task_id}"); os.makedirs(save_dir,exist_ok=True)
        out=join(save_dir,_safe_filename(row["video"] or f"schedule_{row['id']}.mp4"))
        args=["ffmpeg","-hide_banner","-y","-i",url,"-t",str(duration),"-map","0:v:0","-map","0:a?","-c","copy",out]
        proc=await asyncio.create_subprocess_exec(*args)
        _schedule_update(row["id"],"running"); rc=await proc.wait()
        _schedule_update(row["id"],"completed" if rc==0 else "failed")
        if rc==0 and os.path.exists(out): schedule_retention_cleanup(save_dir)
        elif os.path.exists(save_dir): _safe_rmtree(save_dir)
    except asyncio.CancelledError:
        _schedule_update(row["id"],"cancelled")
    except Exception as exc:
        LOG.exception("schedule %s failed: %s",row["id"],exc); _schedule_update(row["id"],"failed")
...
async def _scheduler_loop(client):
    ...
    while True:
        ...
        for row in _schedule_rows():
            ...
            try:
                target=tz.localize(datetime.strptime(f"{row['date']} {row['start_time']}","%d-%m-%Y %H:%M"))
            except Exception: continue
            if target >= now and row["id"] not in _SCHEDULER_TASKS:
                _SCHEDULER_TASKS[row["id"]]=asyncio.create_task(_execute_schedule(client,row))
        await asyncio.sleep(30)


async def _start_scheduler(client):
    if not getattr(_start_scheduler,"started",False):
        _start_scheduler.started=True
        asyncio.create_task(_scheduler_loop(client))


# Start persistent scheduler after the bot is connected.
@app.on_raw_update()
async def _start_persistent_scheduler(client, update, users, chats):
    try: await _start_scheduler(client)
    except Exception: LOG.exception("scheduler startup failed")
...
_BOT_COMMANDS = [
    ...
    BotCommand("Myschedule",     "View/create persistent schedules"),
